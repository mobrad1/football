            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <div class="flex justify-between items-center mb-6">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Player Registration</h3>
                        <a href="{{ route('admin.players') }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">
                            Back to Players
                        </a>
                    </div>

                    <form method="POST" action="{{ route('admin.register-player') }}" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        ...
                        <div>
                            ...
                            <input type="number" name="self_rating" id="self_rating" min="1" max="100" value="{{ old('self_rating') }}" required
                                class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                            @error('self_rating')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="number" name="xp_cost" id="xp_cost" min="1" max="200" value="{{ old('xp_cost') }}" required
                                class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                            @error('xp_cost')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
